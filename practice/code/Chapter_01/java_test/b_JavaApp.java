//Modified StackOverflow version
//수정된파일이긴 한데, 오히려 이파일로 하면 에러남
//pom파일 수정으로 인해서 문제가 해결된듯 합니다.
// 원본이랑 같이 쓰면 이 파일도 통할까? -> 안통함
// 그냥 현재 수정한 것처럼 pom.xml을 수정하면 통하는듯
// 중간에 뭐가 설치된것 같아...
    ...
    public static void main(String[] args) {
        ...
        double totalRevenue = data.map(new DoubleFunction<String[]>() {
            ...
            public double call(String[] strings) throws Exception {
                ...
                //return ret;
                //return Double.parseDouble(strings[2]); // 리턴값이 없음. 이부분도 달라짐
            }
        }).sum();
        ...
        List<Tuple2<String, Integer>> pairs = data.map(new PairFunction<String[], String, Integer>() { // 이부분이 달라짐
            @Override
